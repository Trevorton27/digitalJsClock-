const timeDisplayButton = document.getElementById('timechange-button');
const theMonth = [
  'January',
  'February',
  'March',
...
function displayTime() {
  ...
  const twoDigitMinutes =
    minute < 10 ? (minute.innerText = '0' + minute) : theTime.getMinutes();
  const twoDigitSeconds =
    second < 10 ? (second.innerText = '0' + second) : theTime.getSeconds();
  let currentTime = `${amPmHours}:${twoDigitMinutes}:${twoDigitSeconds}`;
  let currentMiltaryTime = `${hour}:${twoDigitMinutes}:${twoDigitSeconds}`;

  console.log('hour value is: ', hour);
